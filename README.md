...
```bash
hello_aws_lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
hello_aws_lambda$ sam local start-api
hello_aws_lambda$ curl http://localhost:3000/
```
...
```bash
hello_aws_lambda$ sam logs -n HelloWorldFunction --stack-name hello_aws_lambda --tail
```
